...
desired_words_with_diacritics = 1500  # Replace with your desired number of words with diacritics
...
def write_word_and_meta_to_excel(sentences, meta_file):
    ...
    word_df = pd.DataFrame(columns=['Index', 'Sentence Number', 'id', 'Context After', 'Word', 'Context Before'])
    ...
    for i, (sentence, meta_line) in enumerate(zip(sentences, meta_data)):
        ...
        file_name, sentence_number, id = meta_line.strip().split('\t')
        if sentence:
